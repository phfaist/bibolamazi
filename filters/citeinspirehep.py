...
class InspireHEPFetchedAPIInfoCacheAccessor(BibUserCacheAccessor):
    ...
    def parse_and_store_key(self, userkey):
        ...
        if key.startswith('arXiv:'):
            aid = key[len('arXiv:'):]
            if (arxiv2bib.NEW_STYLE.match(aid) or
                arxiv2bib.OLD_STYLE.match(aid)):
                # got an arXiv E-Print reference, save key
                aid = re.sub(r'[^0-9a-zA-Z._-]+', '', aid)
                remember_key(userkey, key=key, p='eprint:'+aid)
                return key

        if key.startswith('doi:'):
            doi = key[len('doi:'):]
            # got a DOI, save key
            remember_key(userkey, key=key, p=invenio_query('doi', doi))
            return key
...
\cite{PhysRev.47.777--EPR+paper}
"""

HELP_TEXT = u"""
This filter scans a LaTeX document for citations of the form
\cite{some-form-of-identifier.99} or \cite{some-form-of-identifier.99--some-comment} and
adds the corresponding bibtex items in the combined bibtex database. The bibtex entry is
generated by querying the InspireHEP API information; you of course need internet access
for this.

The recognized forms of identifiers are:59

    - 'arXiv:<arxiv-id>' -- refer to a particular e-print number

    - 'doi:<doi-identifier>' -- refer to a particular DOI identifier
